        ...
        if($_SERVER['REQUEST_METHOD'] === 'POST'){
            ...
            if($this->model->checkEmail($email)){
                ...
                if($existingUser['id_user'] != $id_user){
                    ...
                    header("Location: edit_user.php?id_user=".$id_user);
                    exit;
                }
            }

            // Panggil method updateUser
            $update = $this->model->updateUser($id_user, $nama_lengkap, $email, $password, $gender, $role);

            if($update){
                $_SESSION['success'] = "User berhasil diupdate!";
                header("Location: list_user.php"); // arahkan ke halaman daftar user
                exit;
            } else {
