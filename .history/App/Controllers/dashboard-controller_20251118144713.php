...
class DASHBOARDController{
    ...
    public function ShowInsertTransaksi(){
        // Ambil data tiket yang masih aktif (belum keluar)
        $listTiket = $this->modelTiket->SelectTiketMasukSaja(); // asumsikan method ini ada di model Tiket
...
public function StoreTransaksi(){
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        if(empty($id_tiket) || $jumlah_bayar <= 0 || empty($metode)){
            ...
            header("Location: ?action=insert-transaksi");
            exit;
        }
