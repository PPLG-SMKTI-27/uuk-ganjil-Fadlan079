...
const hiddenTotal = document.querySelector('input[name="total_harga"]');
hiddenTotal.value = d.harga_flat; // update saat barcode di-scan
...
function hitungDurasi(tglMasuk) {
    let masuk = new Date(tglMasuk);
    let keluar = new Date();

    let diff = Math.floor((keluar - masuk) / 1000);
    ...
    fetch(`?action=get-tiket-by-barcode&barcode=${barcode}`)
        ...
        .then(res => {
            if (res.status === "success") {
                ...
                // disable biar tidak bisa diubah
